// Servicios reales del Hotel Savoia Mendoza — usados en la home (destacados) y en /mendoza/servicios.
export const SERVICIOS = [
  {
    href: '/mendoza/habitaciones',
    src: '/img/mendoza/habitacion1-mendoza.jpg',
    alt: 'Habitación Hotel Savoia Mendoza',
    title: 'Habitaciones',
    description: 'Habitaciones cómodas y equipadas para que disfrutes de un descanso placentero durante tu estadía.',
  },
  {
    href: '/mendoza/pileta',
    src: '/img/mendoza/pileta-mendoza.jpg',
    alt: 'Pileta olímpica exterior',
    title: 'Pileta exterior',
    description: 'Una pileta olímpica exterior, rodeada de verde, ideal para disfrutar.',
  },
  {
    href: '/mendoza/deporte-naturaleza',
    src: '/img/mendoza/espacios-verdes-mendoza.jpg',
    alt: 'Deporte, parques y recreación',
    title: 'Deporte, parques y recreación',
    description: 'Un entorno único rodeado de montañas y espacios verdes, con instalaciones deportivas propias para disfrutar al aire libre.',
  },
  {
    href: '/mendoza/salones-de-eventos',
    src: '/img/mendoza/salon-de-fiesta.jpg',
    alt: 'Salones de eventos',
    title: 'Salones de eventos',
    description: 'Tres salones amplios y versátiles, preparados para reuniones, celebraciones y eventos de todo tipo.',
  },
  {
    href: '/mendoza/gastronomia',
    src: '/img/mendoza/gastronomia-mendoza.jpg',
    alt: 'Gastronomía Savoia',
    title: 'Gastronomía Savoia',
    description: 'Un espacio cálido para disfrutar tus comidas, con la calidez y la calidad que caracterizan a Hoteles Savoia.',
  },
  {
    href: '/mendoza/salon-de-juegos',
    src: '/img/mendoza/salon-de-juegos.jpg',
    alt: 'Salón de juegos',
    title: 'Salón de juegos',
    description: 'Un espacio de recreación con pool y ping pong, ideal para disfrutar en familia o con amigos.',
  },
];
